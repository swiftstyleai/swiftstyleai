...
export function extractMarkdownHeaders(
  ...
  while ((match = headerRegex.exec(markdownContent)) !== null) {
    const level = match && match[1] && match[1].length;
    const headerText = match && match[2];
    if (level && headerText) {
      const t = `h${level}`;
      if (!headers[t]) {
        headers[t] = [];
      }
      headers[t]?.push(headerText.trim());
    }
  }
 ...
 */
export function generateSlug(value?: string, maintainCase = false) {
